...
- name: Update from release
  ...
  with:
    ...
    repo_baseurl_append_path: false
----
...
|Should the value of `to_path` to be appended to `repo_baseurl` if set.
|`true`
...
|`absolute_links`
|When true: Location prefix is set to `https://github.com/${{ inputs.from_repository }}/releases/download/`.

When false: Location prefix is set to `packages/`. This is only useful when using a repository manager that expects files to be relative to the repository metadata. Requires a proxy with url redirecting mechanism to be truly functional.
|`true`
|===
